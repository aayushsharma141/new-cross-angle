...
BEGIN 
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='testimonials') THEN
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='testimonials' AND column_name='created_at') THEN
            EXECUTE 'ALTER TABLE public.testimonials ADD COLUMN created_at timestamptz DEFAULT now();';
        END IF;

        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='testimonials' AND column_name='updated_at') THEN
            EXECUTE 'ALTER TABLE public.testimonials ADD COLUMN updated_at timestamptz DEFAULT now();';
        END IF;
    END IF;
END $$;

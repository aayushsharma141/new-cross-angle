...
| --- | --- | --- | --- | --- | --- |
| **DEF-001** | P1 | DATABASE/RPC | Services create/edit dead since `20260622000002` renamed `icon_url` | 42703 live; `AdminServices.tsx:255` | **[VERIFIED — CLOSED]** (`b4de33d5`, ADR 0004, applied 2026-09-21). Migration `20260915000000` applied to `iuuivmwqodefdrrrewol` via SQL Editor (Dashboard output: "Success. No rows returned"). Probe: `node scripts/checks/probe-upsert-service.mjs` → `PASS — DEF-001 migration is live (new signature resolves, anon revoked)` (both calls HTTP 401 `42501`). CRUD: `verify-def001-services-crud.mjs` → `CREATE rpc: 200 "dc5968d5-5eb1-4969-994f-73fdecdfa905" tag=QA` → `EDIT rpc: 200 tag=QA-EDITED` → `DELETE → gone` → `QA leftovers: []`. `/services` public page renders thumbnails from `asset_usages` (2 images verified via chrome-devtools-mcp snapshot). |
...
| **QA-02** | P1 | DATABASE | `estimate_rates` table absent in production | PGRST205 live; `EstimatorRegistry.ts:20,36`, `useEstimateEngine.ts:31` | **[VERIFIED — CLOSED]** (`77abf233`). Pricing repointed to `estimator_flow_config` key `pricing` via `useFlowConfig`; no DDL. Executed live: save `POST 201` then `200`, row present, restored value intact; public `/estimate` reads `estimator_flow_config?key=eq.pricing`, zero `estimate_rates` calls |
| **QA-03** | P1 | SECURITY | F-07 no rate limiting on `/api/auth/login` | 12×401, no 429 | **[VERIFIED — CLOSED]** (`d0cfc248`, applied 2026-09-23). DB-backed sliding window (`check_and_record_auth_attempt`, migration `20260921000000`) by email (5/15min) and IP (20/15min), fails open on RPC error. Applied by driving the Dashboard SQL Editor directly, one statement at a time, after repeated pasted-block applies silently truncated. Probe: `401,401,401,401,401,429,429,429` → PASS. RPC unit check: `true,true,false,false` at max=2 |
| **QA-04** | P1 | SECURITY | F-08 no Origin/Referer check on auth mutations | cross-origin logout 204 | **[VERIFIED — CLOSED]** (`74dae374`, 2026-09-21). Verified-origin check (`isSameOriginRequest`, `apps/web/api/_lib/security.ts`) on login/logout/refresh/recover — no hardcoded allow-list, compares Origin/Referer against the request's own Host. Executed live: `node scripts/checks/probe-auth-hardening.mjs` — cross-origin `POST /api/auth/logout` → `403` (was `204`). Real-browser regression: `auth-lifecycle-smoke.spec.ts` F-06 and `admin-full-flow.spec.ts` (21/22, one unrelated pre-existing stale-copy failure — QA-09) both pass |
...
| **QA-07** | P2 | UI/ROUTING | Unknown `/admin/*` renders blank | sweep | **[VERIFIED — CLOSED]** (`978c47fc`). `AdminNotFound` as last child of the `/admin` layout route. Executed live: `/admin/cms/blogs`, `/admin/cms/media`, `/admin/does-not-exist` render the not-found state inside the shell; "Back to Hub" → `/admin`; 32/32 real routes unchanged; `admin-interactions` 23/23 |
...
| **QA-10** | P3 | SECURITY (hardening) | `upsert_service` executable by `anon`; only in-function `is_cms_editor()` stops it | P0001 | **[VERIFIED — CLOSED]** (`b4de33d5`, applied 2026-09-21). `REVOKE EXECUTE … FROM PUBLIC, anon` from migration `20260915000000` now live. Probe confirms both old and new call signatures return HTTP 401 `42501 permission denied for function upsert_service`. Anon can no longer reach the in-function guard at all. |

...
| --- | --- | --- | --- | --- | --- |
| **DEF-001** | P1 | DATABASE/RPC | Services create/edit dead since `20260622000002` renamed `icon_url` | 42703 live; `AdminServices.tsx:255` | **[IMPLEMENTED — NOT VERIFIED]** (`b4de33d5`, ADR 0004). Migration `20260915000000` + client committed; **not applied to production** — `scripts/checks/probe-upsert-service.mjs` → FAIL on every run to date (old signature still executes, new one PGRST202). Verified only when the probe passes and `verify-def001-services-crud.mjs` completes create→edit→delete. Runbook: `.planning/tasks/2026-09-15-def001-apply-and-verify.md`. New migration: `upsert_service` without `p_icon_url`; client stops sending it; icon via `asset_usages` |
...
| **QA-10** | P3 | SECURITY (hardening) | `upsert_service` executable by `anon`; only in-function `is_cms_editor()` stops it | P0001 | **[IMPLEMENTED — NOT CLOSED]** `REVOKE EXECUTE … FROM PUBLIC, anon` is in migration `20260915000000`, which is not yet applied; anon still executes the function live (probe: P0001) |

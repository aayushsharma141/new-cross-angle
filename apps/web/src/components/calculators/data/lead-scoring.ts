...
export function scoreLead(data: CalculatorFormData, config: PricingConfig): LeadScore {
    ...
    const budgetScore = scoreBudget(data.budget, weights.budget);
    const scopeScore = scoreScope(data.scopes, data.designPackage, weights.service);
    const areaScore = scoreArea(data.area, weights.area);
    const timelineScore = scoreTimeline(data.timeline, weights.timeline);

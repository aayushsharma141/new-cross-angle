...
export function GalleryCard({
  ...
  return (
    <Card interactive onClick={onClick} className={cn("overflow-hidden group border border-stone-800 bg-stone-950 text-stone-100 relative aspect-square", className)} {...props}>
      {imageUrl ? (
        <img src={imageUrl} alt={title} className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-all duration-[1500ms] ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:scale-[1.02]" />
      ) : (
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-stone-950/90 via-stone-950/20 to-transparent p-6 flex flex-col justify-end transition-opacity duration-300">
        <Heading size="heading-sm" className="text-stone-100 group-hover:text-copper transition-colors">
          {title}
        </Heading>
        {subtitle && (
          <Text size="xs" className="text-stone-400 font-mono mt-1">
            {subtitle}
          </Text>
        )}
      </div>

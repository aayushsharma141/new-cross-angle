  ...
  return (
    <Card interactive onClick={onClick} className={cn("overflow-hidden group border border-subtle", className)} {...props}>
      <div className={cn("w-full bg-stone-900 relative overflow-hidden flex items-center justify-center text-stone-600 font-mono text-xs", aspectStyles[aspectRatio])}>
        {imageUrl ? (
          <img src={imageUrl} alt={title} className="w-full h-full object-cover transition-all duration-[1500ms] ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:scale-[1.02]" />
        ) : (
          <span>[ {title} Render ]</span>
        )}
        <div className="absolute top-3 left-3 z-10">
          <Badge intent="copper">{category}</Badge>
        </div>
      </div>

      <div className="p-6 bg-surface-card">
        <Heading size="heading-sm" className="group-hover:text-copper transition-colors">
          {title}
        </Heading>
        <Text size="sm" variant="secondary" className="mt-1 font-mono">
          {location}
        </Text>
      </div>
    </Card>

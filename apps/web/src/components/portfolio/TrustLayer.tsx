...
export const TrustLayer = () => {
  ...
  return (
    <section className="relative py-10 my-[12vh] border-y border-white/5 bg-background overflow-hidden select-none">
      <style>{`
        @keyframes marquee {
        ...
        <div className="animate-marquee flex gap-16 items-center pr-16 min-w-full">
          {repeatedItems.map((text, i) => (
            <React.Fragment key={i}>
              <span className="text-[10px] tracking-[0.25em] font-mono text-white/30 uppercase">
                {text}
              </span>

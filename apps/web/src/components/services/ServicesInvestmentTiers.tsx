...
const tiers = [
  ...
  },
  {
    ...
    accent: "border-site-gold/40 hover:border-site-gold/70 ring-1 ring-site-gold/15",
    bg: "bg-[#0d0b06]",
    labelColor: "text-site-gold",
    featured: true,
  },
  {
    id: "bespoke",
    ...
    accent: "border-site-crimson/30 hover:border-site-crimson/55",
    bg: "bg-[#0a0606]",
    labelColor: "text-site-crimson",
    featured: false,
  },
];
...
const ServicesInvestmentTiers = () => (
  <section className="relative bg-[#040404] py-24 lg:py-36 px-6 overflow-hidden border-b border-white/[0.04]">
    {/* Subtle radial glow */}
    <div className="absolute inset-0 pointer-events-none bg-[radial-gradient(ellipse_60%_50%_at_50%_100%,rgba(196,18,48,0.05),transparent)]" />
    ...
    <div className="max-w-[1400px] mx-auto relative z-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-12">
        {tiers.map((tier, i) => (
          ...
          >
            {tier.featured && (
              <div className="absolute -top-px left-8 bg-site-gold px-4 py-1 text-[8px] font-bold uppercase tracking-[0.3em] text-black">
                Most Popular
              </div>
            )}
            ...
            <ul className="space-y-2.5 mb-0">
              {tier.includes.map((item) => (
                <li key={item} className="flex items-start gap-2.5 text-[0.8rem] text-white/55 font-light">
                  <div className={`mt-[0.45em] w-[5px] h-[5px] rounded-full shrink-0 ${tier.featured ? "bg-site-gold" : "bg-white/30"}`} />
                  {item}
                </li>
              ))}
            </ul>
      ...
      <motion.div
        ...
        className="flex flex-col sm:flex-row items-center justify-between gap-6 border border-white/[0.07] bg-white/[0.02] px-8 py-6"
      >
        ...
        <Link
          to="/estimate"
          className="group shrink-0 inline-flex items-center gap-3 bg-site-crimson text-white font-bold uppercase tracking-[0.18em] text-[10px] px-8 py-4 transition-all duration-300 hover:bg-site-crimson/85 hover:shadow-[0_10px_30px_rgba(196,18,48,0.3)]"
        >

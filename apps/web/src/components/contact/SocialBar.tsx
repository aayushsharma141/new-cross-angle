...
const socialMeta: Record<string, { icon: React.ReactNode; name: string; handle?: string }> = {
  instagram: {
    name: "Instagram",
    handle: "@crossangleinterior",
        ...
        <div className="flex flex-wrap items-center justify-center gap-3">
          {socialLinks.map((social) => (
            <a
              ...
              className="contact-social-link group flex items-center gap-2.5 rounded-full border border-white/[0.08] bg-white/[0.03] px-4 py-2.5 text-[11px] font-semibold uppercase tracking-[0.18em] text-white/45 backdrop-blur-sm transition-all duration-300 hover:border-[#d1af6e]/30 hover:bg-[#d1af6e]/[0.06] hover:text-[#d1af6e]/80"
            >
              <span className="text-white/30 group-hover:text-[#d1af6e]/60 transition-colors">
                {social.icon}
              </span>
              {social.name}
            </a>

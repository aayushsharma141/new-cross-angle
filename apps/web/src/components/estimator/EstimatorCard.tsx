...
const EstimatorCard: React.FC<EstimatorCardProps> = ({ title, description, icon, onClick }) => {
  return (
    <button 
      onClick={onClick}
      className="group flex flex-col items-center justify-center gap-4 w-full h-full p-8 text-center bg-white/40 backdrop-blur-xl border border-white/60 shadow-[0_8px_30px_rgba(0,0,0,0.04)] rounded-3xl hover:bg-white/70 hover:shadow-[0_12px_40px_rgba(0,0,0,0.08)] hover:-translate-y-2 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-[#8b6f47] transition-all duration-500 relative overflow-hidden"

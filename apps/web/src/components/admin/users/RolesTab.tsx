  ...
  <div className="space-y-6">
    <div className="grid gap-4 lg:grid-cols-3">
      {(["super_admin", "admin", "viewer"] as const).map((role) => (
        <Card key={role} className="border-zinc-800/60 bg-zinc-900/50 text-zinc-100">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 font-serif text-2xl"><Shield className="h-5 w-5 text-primary" />{ROLE_LABELS[role]}</CardTitle>
    ...
    <Card className="border-zinc-800/60 bg-zinc-900/50 text-zinc-100">
      <CardHeader>
        ...
        <CardDescription className="text-zinc-400">These rules are enforced in the UI and in the edge functions.</CardDescription>
      </CardHeader>

...
export const Container = ({ className, ...rest }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn("mx-auto w-full max-w-[1440px] px-6 md:px-10 lg:px-16 xl:px-20", className)} {...rest} />
);
...
/**
 * Numbered hairline accordion, one panel open at a time.
 * `idPrefix` keeps trigger/panel ids unique when a page has several.
 */
export const FaqAccordion = ({ items, idPrefix }: { items: FaqItem[]; idPrefix: string }) => {
          ...
          <motion.li key={faq.question} {...reveal(index * 0.04)} className="border-t border-white/10 last:border-b">
            ...
            >
              <span className="grid grid-cols-[2.5rem_1fr] gap-4">
                ...
                <span
                  className={cn(
                    "font-display text-xl transition-colors duration-300 md:text-2xl group-focus-visible:text-primary",
              ...
              <span
                aria-hidden="true"
                className={cn(
                  "mt-2 shrink-0 text-lg font-light leading-none text-white/40 transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]",
                  isOpen && "rotate-45",
                )}
              >
                +

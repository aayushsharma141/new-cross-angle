...
interface Props {
  onComplete: (data: Pick<UserSignals, 'constraints'>) => void;
}

const DISLIKE_COLORS = ["White", "Black", "Beige", "Grey", "Pink", "Yellow", "Red", "Blue"];

export default function Constraints({ onComplete }: Props) {
  ...
  const handleSubmit = () => {
    onComplete({ constraints: selectedDislikes });
  };
  ...
  return (
    <div className="relative flex min-h-[85vh] w-full flex-col items-center justify-center px-4">
      <div className="w-full max-w-3xl mx-auto flex flex-col items-center">
        ...
        >
          <p className="text-xs font-bold tracking-[0.15em] text-[#5a5a5a] uppercase mb-4">CONSTRAINTS</p>
          ...
          <p className="text-[#5a5a5a] mb-8 text-base font-light">
            Select the colors you absolutely dislike.
          </p>
          ...
          <div className="flex flex-wrap justify-center gap-3 w-full mb-10">
            {DISLIKE_COLORS.map((color) => (
              <button
                ...
                className={`px-6 py-3 rounded-full border transition-all duration-300 text-sm font-medium font-serif focus-visible:ring-2 focus-visible:ring-[#80643e] focus-visible:outline-none ${

...
export const DiscoveryProgressSidebar = ({
    ...
    return (
        <aside aria-label="Discovery progress" className="hidden md:flex flex-col bg-transparent border-r border-[var(--s-border-subtle)] p-8 sticky top-0 h-[100dvh] overflow-y-auto w-[280px] shrink-0">
            <div className="flex items-center gap-2 text-[14px] tracking-[0.08em] uppercase text-[var(--s-accent-primary)] font-semibold mb-8">
            ...
            <div className="mb-6 relative">
                <div className="flex items-center justify-between mb-2">
                    <span className="text-[10px] font-mono tracking-[0.2em] uppercase text-[var(--s-text-muted)]">Journey</span>
                    <motion.span
                        key={progress}
                        aria-live="polite"
                        initial={{ opacity: 0, y: 4 }}
                        animate={{ opacity: 1, y: 0 }}
                        className="text-[10px] font-mono text-[var(--s-accent-primary)] font-semibold"
                    >
                        {progress}%
                    </motion.span>
                </div>
                <div className="h-[2px] bg-[var(--s-border-subtle)] relative overflow-hidden rounded-full">
                    <motion.div
                        className="absolute inset-y-0 left-0 bg-[var(--s-accent-primary)]"
                        initial={{ width: 0 }}
            ...
            <div className="relative">
                {/* Base vertical line */}
                <div className="absolute left-[10px] top-[21px] bottom-[21px] w-[2px] bg-[var(--s-border-subtle)] -z-10" aria-hidden="true" />
                {/* Active vertical line */}
                <motion.div 
                    className="absolute left-[10px] top-[21px] w-[2px] bg-[var(--s-accent-primary)] origin-top -z-10"
                    initial={{ scaleY: 0 }}
                    animate={{ 
                        scaleY: (() => {
                    ...
                    return (
                        ...
                        >
                            <button
                                ...
                                className={cn(
                                    "w-full text-left transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--s-accent-primary)] focus-visible:ring-offset-2 rounded-md flex flex-col gap-2 bg-transparent border-0 p-0",
                                    isClickable ? "cursor-pointer group" : "cursor-default"
                                )}
                            >
                                <div className="flex items-center gap-3">
                                    <div className={cn(
                                        "w-[22px] h-[22px] rounded-full border-[1.5px] flex items-center justify-center text-[11px] font-semibold shrink-0 transition-all duration-300 bg-[var(--s-surface-raised)]",
                                        isActive 
                                            ? "border-[var(--s-accent-primary)] bg-[var(--s-accent-primary)] text-white" 
                                            : isCompleted 
                                                ? "border-[var(--s-accent-primary)] text-[var(--s-accent-primary)]" 
                                                : "border-[var(--s-border-subtle)] text-[var(--s-text-muted)]",
                                        isClickable && !isActive && "group-hover:border-[var(--s-accent-primary)] group-hover:text-[var(--s-accent-primary)]"
                                    )} aria-hidden="true">
                                        {isCompleted ? "✓" : parseInt(eyebrow, 10)}
                                    </div>
                                    <span className={cn(
                                        "text-[14px] transition-colors",
                                        isActive ? "text-[var(--s-text-primary)] font-semibold" : isCompleted ? "text-[var(--s-accent-primary)]" : "text-[var(--s-text-muted)]",
                                        isClickable && !isActive && "group-hover:text-[var(--s-accent-primary)]"
                                    )}>{label}</span>
                                </div>
                            </button>
                                            ...
                                            return (
                                                <div 
                                                    ...
                                                    className={cn(
                                                        ...
                                                        isSubActive || isSubCompleted ? "bg-[var(--s-accent-primary)]" : "bg-[var(--s-border-subtle)]"
                                                    )}
                                                />
                                            );
                                        })}
            ...
            <AnimatePresence>
                {archetype && (
                    <motion.div
                        ...
                        exit={{ opacity: 0 }}
                        aria-live="polite"
                        className="shrink-0 mt-6 p-4 border border-[var(--s-border-subtle)] bg-[var(--s-surface-sunken)] rounded-[8px] relative overflow-hidden"
                    >
                        <span className="text-[10px] font-mono tracking-[0.2em] uppercase text-[var(--s-accent-primary)] font-semibold block mb-1">
                            Emerging Profile
                        </span>
                        <span className="text-[14px] font-semibold text-[var(--s-text-primary)] block font-serif">
                            {archetype}
                        </span>
                    </motion.div>
                )}
            ...
            <div className="mt-8 pt-4 border-t border-[var(--s-border-subtle)]">
                <p className="text-[10px] text-[var(--s-text-muted)] leading-relaxed tracking-wide">

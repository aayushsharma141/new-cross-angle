...
import { SecurityTab } from "@/components/admin/users/SecurityTab";
...
export default function AdminUsers(): JSX.Element {
  ...
  const initialTab = searchParams.get("tab") === "security" ? "security" : searchParams.get("tab") === "roles" ? "roles" : "users";
  ...
  const handleTabChange = (nextTab: string) => {
    ...
    if (nextTab === "users") nextParams.delete("tab");
    ...
    <div className="flex flex-col space-y-6 animate-in fade-in duration-700">
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
        <TabsList className="w-fit gap-1 rounded-2xl border border-zinc-800 bg-zinc-900/40 p-1">
          <TabsTrigger value="users" className="rounded-xl px-5 py-2.5 text-xs font-bold uppercase tracking-widest data-[state=active]:bg-primary data-[state=active]:text-white">Users</TabsTrigger>
          <TabsTrigger value="roles" className="rounded-xl px-5 py-2.5 text-xs font-bold uppercase tracking-widest data-[state=active]:bg-primary data-[state=active]:text-white">Roles &amp; Access</TabsTrigger>
          <TabsTrigger value="security" className="rounded-xl px-5 py-2.5 text-xs font-bold uppercase tracking-widest data-[state=active]:bg-primary data-[state=active]:text-white">Security</TabsTrigger>
        </TabsList>
